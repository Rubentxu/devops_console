...
func (e *K8sTaskExecutor) runTask(ctx context.Context, task *entities.DevOpsTask, taskExecution *entities.TaskExecution) {
	...
	// Almacenar detalles específicos del ejecutor
	taskExecution.ExecutionDetails = map[string]interface{}{
		"PodName": podName,
	}
	...
	// Publicar evento que el pod está en ejecución
	e.publishEvent(taskExecution.ID, entities.EventTypePodName, podName)
...
func (e *K8sTaskExecutor) WaitForPodReady(ctx context.Context, podName string) error {
	podsClient := e.clientset.CoreV1().Pods(e.namespace)

	return wait.PollUntilContextTimeout(ctx, time.Second, 5*time.Minute, true, func(context.Context) (bool, error) {
		...
		case corev1.PodRunning:
			return true, nil
		case corev1.PodFailed, corev1.PodSucceeded:
			return false, fmt.Errorf("pod terminated before running")
		default:
			return false, nil
		}
	})
...
func (e *K8sTaskExecutor) UpdatePodCommand(ctx context.Context, podName string, newCommand []string) error {
	podsClient := e.clientset.CoreV1().Pods(e.namespace)
	...
	// Create a new pod with the updated command
	newPod := pod.DeepCopy()
	newPod.ResourceVersion = ""
	newPod.Spec.Containers[0].Command = newCommand

	_, err = podsClient.Create(ctx, newPod, metav1.CreateOptions{})
	if err != nil {
		return fmt.Errorf("error creating new pod: %v", err)
	}
