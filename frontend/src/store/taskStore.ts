...
export const useTaskStore = create<TaskStore>((set, get) => ({
  ...
  getTaskStatus: async (taskId) => {
    try {
      return await GetTaskStatus(taskId) as TaskStatus;
    } catch (error) {
      console.error("Error getting task status:", error);
      return "unknown" as TaskStatus;
    }
  },
...
EventsOn("task:execution:added", (data: { taskId: string, execution: any }) => {
  useTaskStore.setState(state => ({
    tasks: state.tasks.map(t => {
      if (t.id === data.taskId) {
        return { ...t, executions: [...(t.executions || []), data.execution] };
      }
      return t;
    })
  }));
...
EventsOn("task:execution:error", (data: { taskId: string, error: string }) => {
  useTaskStore.setState(state => ({
    taskExecution: state.taskExecution && state.taskExecution.taskId === data.taskId
      ? { ...state.taskExecution, status: "failed" as TaskStatus, error: data.error }
      : state.taskExecution
  }));
  useTaskStore.getState().updateTaskStats("failed" as TaskStatus);
});
